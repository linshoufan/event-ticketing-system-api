...
def test_no_key_returns_422(client, make_user):
...
def test_wrong_key_returns_401(client, make_user):
    user = make_user()
    response = client.get(
...
def test_registration_profile_returns_all_fields(client, make_user):
    user = make_user(role="employee", registration_status="active")
    response = client.get(
        f"/v1/internal/users/{user.user_id}/registration-profile",
...
def test_punish_active_user_becomes_locked(client, make_user):
    user = make_user(role="employee", registration_status="active")
...
def test_punish_already_locked_user_resets_unlock_at(client, make_user):
    user = make_user(role="employee", registration_status="locked")
    # punish 第一次
    client.post(
        f"/v1/internal/users/{user.user_id}/punish",
        headers={INTERNAL_KEY_HEADER: VALID_KEY},
