...
gcloud run deploy account-service \
  ...
  --set-env-vars "SERVICE=account,ENV=production,ACCOUNT_DB_HOST=/cloudsql/PROJECT_ID:REGION:INSTANCE_NAME,ACCOUNT_DB_PORT=5432,ACCOUNT_DB_USER=postgres,ACCOUNT_DB_PASSWORD=CHANGE_ME,ACCOUNT_DB_NAME=account_db,JWT_SECRET_KEY=CHANGE_ME,JWT_ALGORITHM=HS256,INTERNAL_API_KEY=CHANGE_ME"
```
